 ...
 *          To mutate the code it's used a Rewrite object, that represent the
 * AST of a translation unit.
 *          The mutator can be a First Order Mutation (FOM) or High Order (HOM).
 * The default is First Order.
 * @details Some times the mutation operation to be valid requires additional
...
public:
  ...
  MutatorType getTypes() const { return types; }

  void setTypes(MutatorType types) { this->types = types; }

  const MatcherType getMatcherType() const { return matcherType; }

  const bool isHom() const { return isHOM; }
   ...
   */
  virtual bool match(const NodeType &node) { return true; }
  /**
   * @}
   */
